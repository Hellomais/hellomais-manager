...
export default function UserFormModal({ isOpen, onClose, user = null, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const loadingToast = toast.loading(user ? 'Atualizando usuário...' : 'Criando usuário...');
    ...
    try {
      ...
      if (user) {
        ...
        // Para PATCH: eventos como string separada por vírgula
        const eventIds = formData.events
          .filter(e => e !== null && e !== undefined)
          .map(e => String(e).replace(/[^0-9]/g, ''))
          .filter(e => e !== '')
          .join(',');

        formDataToSend.append('events', eventIds);
        ...
        // POST
        // Para POST: eventos como array de strings
        const cleanEvents = formData.events
          .filter(e => e !== null && e !== undefined)
          .map(e => String(e).replace(/[^0-9]/g, ''))
          .filter(e => e !== '');
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || (user ? 'Erro ao atualizar usuário' : 'Erro ao criar usuário'));
      }
